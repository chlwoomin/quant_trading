...
분석 원칙:
1. 먼저 전략이 벤치마크(KOSPI)를 이기고 있는지, 절대수익과 상대수익을 분리해서 판단하세요.
2. 성과 부진의 원인을 팩터 가중치, 동일비중 구조, 현금비중, 종목 선정, 시장 쏠림, 리스크 오버레이 중 어디에 가까운지 진단하세요.
3. 작은 변화를 권장하세요. 한 번에 가중치를 ±10%p 이상 바꾸지 마세요.
4. 최근 시장 레짐(KOSPI 상황), 과열 여부, KOSPI 대비 상대성과를 반드시 고려하세요.
5. 데이터가 부족하면(백테스트 기간 < 1년) should_update=false를 반환하세요.
6. 검증 지표(Sharpe, MDD, Alpha)가 이미 양호하면 변경을 자제하되, 모니터링 항목을 구체적으로 제시하세요.
7. 전략 변경 제안과 운용상 관찰/주의사항을 구분하세요. 변경하지 않더라도 진단은 충분히 길게 작성하세요.

응답은 반드시 다음 JSON 형식으로만 답하세요. 다른 텍스트는 포함하지 마세요:
{
  "should_update": true 또는 false,
  "reasoning": "최종 판단 요약 (한국어, 4-6문장)",
  "market_assessment": "현재 시장 상황 평가 (한국어, 2-3문장)",
  "benchmark_assessment": "KOSPI 대비 상대성과 평가 (한국어, 2-3문장)",
  "risk_assessment": "리스크/집중도/현금비중/스탑로스 관점 평가 (한국어, 2-3문장)",
...
def _build_user_message(config: dict, backtest_metrics: dict, portfolio_perf: dict,
    ...
    if review_context:
        live_alpha = review_context.get("live_alpha", {})
        allocation = review_context.get("allocation", {})
        flags = review_context.get("diagnosis_flags", [])
        risk = review_context.get("risk_signal", {})
        lines += [
            "",
            "## 실전 운용 진단 컨텍스트",
            f"시장 레짐: {risk.get('signal', 'N/A')} / KOSPI 200MA 괴리 {risk.get('gap_pct', 0):+.2f}% / 데이터 소스 {risk.get('source', 'N/A')}",
            f"현금 비중: {allocation.get('cash_ratio_pct', 0):.2f}% / 주식 비중: {allocation.get('stock_ratio_pct', 0):.2f}% / 상위 5종목 비중: {allocation.get('top5_weight_pct', 0):.2f}%",
        ]
        if live_alpha:
            lines += [
                f"실전 기간: {live_alpha.get('start_date', 'N/A')} ~ {live_alpha.get('end_date', 'N/A')}",
                f"실전 포트폴리오 수익률: {live_alpha.get('portfolio_return_pct', 0):+.2f}%",
                f"동기간 KOSPI 수익률: {live_alpha.get('kospi_return_pct', 0):+.2f}%",
                f"실전 KOSPI 대비 상대수익률: {live_alpha.get('relative_alpha_pct', 0):+.2f}%p",
            ]
        if flags:
            lines.append("진단 플래그: " + ", ".join(flags))

        top = review_context.get("top_contributors", [])
        bottom = review_context.get("bottom_contributors", [])
        if top:
            lines += ["", "## 보유 종목 기여 상위"]
            for h in top:
                lines.append(
                    f"- {h['name']}({h['ticker']}): 비중 {h['weight_pct']:.2f}%, "
                    f"수익률 {h['pnl_pct']:+.2f}%, 손익 {h['pnl_amt']:+,.0f}원"
                )
        if bottom:
            lines += ["", "## 보유 종목 기여 하위"]
            for h in bottom:
    ...
    lines += [
        ...
        "",
        "위 정보를 바탕으로 전략 파라미터 조정 여부, 성과 원인, KOSPI 대비 판단, 다음 액션을 구체적으로 작성하고 JSON 응답을 반환하세요.",
    ]
...
def analyze_and_suggest(
    ...
    user_msg = _build_user_message(config, backtest_metrics, portfolio_perf,
                                   regime_history, review_context)
    ...
    try:
        ...
        return {
            ...
            "benchmark_assessment": parsed.get("benchmark_assessment", ""),
            "risk_assessment": parsed.get("risk_assessment", ""),
            "performance_diagnosis": parsed.get("performance_diagnosis", []),
...
def _fallback_analysis(config: dict, backtest_metrics: dict) -> dict:
    ...
    return {
        ...
        "benchmark_assessment": "API 미연동으로 KOSPI 대비 정성 평가 불가",
        "risk_assessment": "API 미연동으로 리스크 정성 평가 불가",
        "performance_diagnosis": [reason],
        "action_items": ["Claude API 연결 상태 확인", "백테스트와 실전 KOSPI 대비 수익률 재점검"],
        "watchlist": ["KOSPI 대비 상대수익률", "Sharpe", "MDD"],
        "confidence": "low",
        "suggested_config": suggested_config,
